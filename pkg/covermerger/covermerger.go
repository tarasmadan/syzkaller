...
func mergedCoverageRecords(fmr *FileMergeResult) ([]*coveragedb.MergedCoverageRecord, []*coveragedb.FuncLines) {
	...
	for _, line := range lines {
		...
		if funcName := checkedFuncName(srcFuncs); funcName != "" {
...
// checkedFuncName gives the best function name candidate.
// You can think about the case where some function was renamed.
//
// The better alternative is to get the function name from the C code. But it looks more complex for now.
func checkedFuncName(names []string) string {
	stat := map[string]int{}
